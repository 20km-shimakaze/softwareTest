...
public class HManaFlight {
	...
	*/
	...
	{         
		...
		// 遍历整个ArrayList         
		for (HFlightInfo hFlightInfo : m_listFlight) {
			// 取出HFlightInfo对象
			// 比较姓名
			if (((HFlightInfo) hFlightInfo).getStrName().equals(strName)) {
				sbInfo = new StringBuilder();
				sbInfo.append(((HFlightInfo) hFlightInfo).GetInfo()).append("\r\n");
				break;
			}
		} //end of for
		assert sbInfo != null;
	...
	private  boolean ReadFlightFile()
	{
		...
		try{
			DataInputStream fm = new DataInputStream(new BufferedInputStream(
					new FileInputStream("com/Flight/FlightInfo.txt")));
			//航班个数
	...
	public  boolean WriteFlightFile()
	{
		...
		try{
			DataOutputStream fm = new DataOutputStream(new BufferedOutputStream(
					new FileOutputStream("com/Flight/FlightInfo.txt")));
			//航班个数
	...
	public String SearchByCompanyName(String strCompany)
	{
		...
		for (HFlightInfo hFlightInfo : m_listFlight) {
			// 取出HFlightInfo对象
			String strTmp;
			if (((HFlightInfo) hFlightInfo).getStrCompany().equals(strCompany)) {
				strTmp = ((HFlightInfo) hFlightInfo).GetInfo();
				sbInfo.append(strTmp).append("\r\n");
			}
		} //end of for
		return sbInfo.toString();
	}
